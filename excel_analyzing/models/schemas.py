...
class ColumnInfo(BaseModel):
    ...
    is_nullable: bool = Field(default=True, description="Whether column can contain null values")
...
class SheetInfo(BaseModel):
    ...
    has_header: bool = Field(default=True, description="Whether first row contains headers")
    header_row: int = Field(default=0, ge=0, description="Zero-based header row index")
    data_start_row: int = Field(default=1, ge=0, description="Zero-based data start row index")
...
class WorkbookInfo(BaseModel):
    ...
    sheets: List[SheetInfo] = Field(default_factory=list, description="Sheet information")
    created_at: datetime = Field(default_factory=datetime.utcnow)
    processed_at: Optional[datetime] = Field(None, description="When workbook was processed")
...
class ProcessingOptions(BaseModel):
    ...
    drop_empty_rows: bool = Field(default=True, description="Drop completely empty rows")
    drop_empty_columns: bool = Field(default=True, description="Drop completely empty columns")
    infer_data_types: bool = Field(default=True, description="Automatically infer column data types")
    clean_column_names: bool = Field(default=True, description="Clean and normalize column names")
    max_sample_size: int = Field(default=100, ge=1, description="Maximum sample size for type inference")
    null_threshold: float = Field(default=0.9, ge=0, le=1, description="Threshold for dropping columns with too many nulls")
...
class ProcessingResult(BaseModel):
    ...
    rows_processed: int = Field(default=0, ge=0, description="Total rows processed across all sheets")
    columns_processed: int = Field(default=0, ge=0, description="Total columns processed across all sheets")
    processing_time_seconds: float = Field(default=0, ge=0, description="Processing time in seconds")
    created_at: datetime = Field(default_factory=datetime.utcnow)
