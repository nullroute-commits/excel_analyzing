...
class ExcelPipeline:
    ...
    def process_directory(
        ...
        logger.info(
            f"Directory processing complete: {successful_count} successful, "
    ...
    def _save_workbook_to_database(self, workbook_info: WorkbookInfo) -> None:
        ...
        try:
            ...
            existing_workbook = (
                session.query(WorkbookModel)
                .filter_by(file_path=str(workbook_info.file_path))
                .first()
            )
    ...
    def _save_processing_result_to_database(self, result: ProcessingResult) -> None:
        ...
        try:
            ...
            workbook_model = (
                session.query(WorkbookModel)
                .filter_by(file_path=str(result.workbook.file_path))
                .first()
            )
            ...
            logger.debug(
                f"Saved processing result to database: {result.workbook.file_name}"
            )
    ...
    def get_workbook_summary(self, file_path: Path) -> Optional[Dict]:
        ...
        try:
            workbook_model = (
                session.query(WorkbookModel).filter_by(file_path=str(file_path)).first()
            )
            ...
            return {
                ...
                "processed_at": (
                    workbook_model.processed_at.isoformat()
                    if workbook_model.processed_at
                    else None
                ),
                "sheets": sheets_info,
            }
