...
class ExcelDataProcessor:
    """Pandas wrapper class for processing Excel data with filters/transformations."""
    ...
    def _process_sheet(self, file_path: Path, sheet_name: str) -> SheetInfo:
        ...
        if self.options.null_threshold < 1.0:
            ...
            columns_to_keep = null_percentages[
                null_percentages <= self.options.null_threshold
            ].index
        ...
        for i, col_name in enumerate(df_data.columns):
            ...
            col_info = self._analyze_column(
                df_data[col_name], col_name, original_name, i
            )
    ...
    def _analyze_column(
        self, series: pd.Series, name: str, original_name: str, position: int
    ) -> ColumnInfo:
        ...
        # Infer data type
        data_type = (
            self._infer_data_type(non_null_series)
            if self.options.infer_data_types
        ...
        sample_values = (
            non_null_series.head(sample_size).tolist()
    ...
    def _infer_data_type(self, series: pd.Series) -> DataType:
        ...
        if (
            series.dtype == "bool"
            or series.isin([True, False, "True", "False", "true", "false"]).all()
        ):
        ...
        try:
            ...
            if dt_series.dt.time.apply(
                lambda x: x == pd.Timestamp("00:00:00").time()
            ).all():
    ...
    def get_dataframe(
        self, workbook_name: str, sheet_name: str
    ...
    def apply_filter(
        self, workbook_name: str, sheet_name: str, filter_condition: str
    ) -> Optional[pd.DataFrame]:
    ...
    def apply_transformation(
        self, workbook_name: str, sheet_name: str, transformation: Dict[str, Any]
    ) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if transformation.get("operation") == "group_by":
                ...
                logger.warning(
                    ...
                    f"{transformation.get('operation')}"
                )
                return df
